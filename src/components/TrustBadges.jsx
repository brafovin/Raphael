  ...
  return (
    <div className={`grid grid-cols-2 lg:grid-cols-4 gap-4 ${className}`}>
      {badges.map((badge) => (
        <div
          key={badge.title}
          className="flex flex-col items-center text-center gap-2 p-4 rounded-xl bg-dark-card border border-dark-border"
        >
          <span className="text-neon">{badge.icon}</span>
          <div>
            <p className="text-white text-sm font-semibold">{badge.title}</p>
            <p className="text-zinc-500 text-xs mt-0.5">{badge.text}</p>
          </div>
        </div>
      ))}
    </div>
